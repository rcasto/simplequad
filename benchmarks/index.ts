...
// Seeds used to average game-loop results across different random layouts.
const SEEDS = [42, 137, 601, 1337, 4538, 9999, 13529, 54321, 72345, 98765];

function parseSeed(): number {
  const args = process.argv.slice(2);
  for (let i = 0; i < args.length; i++) {
    if (args[i].startsWith("--seed="))
      return parseInt(args[i].slice("--seed=".length), 10);
...
console.log(
  `  seed: ${seed}  (game-loop averaged across seeds: ${SEEDS.join(", ")})`,
);

// Insert/query/remove/stress use a single seed for reproducibility.
// Game-loop uses multiple seeds and averages for more stable headline numbers.
runInsertBenchmarks(seededRandom(seed));
runQueryBenchmarks(seededRandom(seed));
runRemoveBenchmarks(seededRandom(seed));
runGameloopBenchmarks(SEEDS);
...
console.log(
  `  Results saved to benchmarks/results/benchmark_${datestamp}_${timestamp}.txt`,
);
